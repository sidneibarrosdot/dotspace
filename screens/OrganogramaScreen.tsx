...
const OrganogramaScreen: React.FC<OrganogramaScreenProps> = ({
  ...
  return <div className={light ? 'min-h-screen bg-gray-100 text-zinc-900' : 'min-h-screen bg-[#0d0e10] text-white'}>
    <Header theme={theme} toggleTheme={toggleTheme} isLoggedIn={isLoggedIn} sessionActive={Boolean(user)} canManageAdmin={isLoggedIn} offlineMode={offlineMode} onNavigateToAdmin={onNavigateToAdmin} onLogout={onLogout} />
    <main className="container mx-auto px-4 py-6 pb-44 sm:px-6 sm:py-8 lg:px-8">
      <section className="grid gap-6 xl:grid-cols-[280px_minmax(0,1fr)]">
        ...
        <div className="space-y-6"><section className={`relative overflow-hidden rounded-[34px] border p-8 ${light ? 'border-zinc-200 bg-white' : 'border-white/15 bg-gradient-to-br from-[#222329] to-[#151618]'}`}><p className="text-xs font-bold uppercase tracking-[0.35em] text-[#88C125]">Estrutura organizacional</p><h1 className="mt-3 text-3xl font-black sm:text-4xl">Conheça nossos times</h1><p className={`mt-4 max-w-2xl leading-7 ${light ? 'text-zinc-600' : 'text-white/70'}`}>Visualize coordenações, lideranças, subáreas e colaboradores. Os dados são atualizados pela planilha de governança.</p></section><OrgChart theme={theme} /></div>
      </section>
    </main>
