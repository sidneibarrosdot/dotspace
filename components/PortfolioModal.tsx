                    ...
                    <div className="p-8">
                        ...
                        {!isEditing && !isCreating && (
                            <div className="flex flex-col gap-4 mb-8">
                                <div className="grid grid-cols-2 gap-2 text-sm text-gray-500 dark:text-gray-400 sm:flex sm:flex-wrap sm:items-center sm:gap-4">
                                    <div className="flex min-w-0 items-center gap-1.5 rounded-lg bg-gray-50 px-3 py-2 dark:bg-zinc-700/40">
                                        <EyeIcon className="h-4 w-4 shrink-0" />
                                        <span className="min-w-0 truncate">{editableItem.views || 0} visualizações</span>
                                    </div>
                                    <button 
                                        onClick={async () => {
                                            if (onLike) onLike(editableItem);
                                        }}
                                        className={`flex min-w-0 items-center justify-center gap-1.5 rounded-lg bg-gray-50 px-3 py-2 text-sm transition-colors cursor-pointer dark:bg-zinc-700/40 ${
                                            isLiked ? 'text-accent' : 'hover:text-accent'
                                        }`}
                                        title={isLiked ? "Descurtir" : "Curtir"}
                                    >
                                        <HeartIcon className={`h-4 w-4 shrink-0 ${isLiked ? 'fill-current' : ''}`} />
                                        <span className="min-w-0 truncate">{editableItem.likes || 0} curtidas</span>
                                    </button>
                                    
                                    {isLoggedIn && onToggleFavorite && (
                                        <button 
                                            onClick={() => onToggleFavorite(editableItem)}
                                            className={`col-span-2 flex min-w-0 items-center justify-center gap-1.5 rounded-lg bg-gray-50 px-3 py-2 text-sm transition-colors cursor-pointer dark:bg-zinc-700/40 sm:col-span-1 ${
                                                isFavorited ? 'text-accent' : 'hover:text-accent'
                                            }`}
                                        >
                                            <Bookmark className={`h-4 w-4 shrink-0 ${isFavorited ? 'fill-current' : ''}`} />
                                            <span className="min-w-0 truncate">{isFavorited ? 'Salvo nos Favoritos' : 'Salvar para ver depois'}</span>
                                        </button>
                                    )}
                                </div>
                                ...
                                {editableItem.Link_PMV && editableItem.Link_PMV !== '#' && (
                                    <a 
                                        ...
                                        className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-accent px-6 py-2.5 font-bold text-white shadow-md transition-all duration-300 hover:-translate-y-0.5 hover:bg-accent-dark hover:shadow-lg dark:text-zinc-900 sm:w-auto"
                                    >
                                        Acessar PMV
